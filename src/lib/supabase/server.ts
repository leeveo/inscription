...
// Specific function for API routes with Request object
export async function supabaseApiRoute(request: Request) {
  const cookieStore = await cookies()
      ...
      cookies: {
        ...
        setAll(cookiesToSet) {
          try {
            ...
            console.log('Cookie set error (expected in API routes):', error)
          }
        },
      },
    }
...
// Alternative function for API routes that need auth but handles it better
export async function supabaseAuthenticatedApi() {
  const cookieStore = await cookies()
