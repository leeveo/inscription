-- Création de la table pour les check-ins des participants
-- Compatible Supabase

-- Création de la table pour les check-ins
CREATE TABLE IF NOT EXISTS inscription_checkins (
  ...
  checked_by TEXT, -- Nom ou ID de l'hôtesse qui a fait le check-in
  qr_token TEXT NOT NULL, -- Token unique pour vérifier l'authenticité du QR code
  device_info JSONB, -- Info sur l'appareil utilisé pour le check-in
  ...
  -- Un participant ne peut faire qu'un seul check-in par session
...
-- Table pour les tokens QR uniques des participants
CREATE TABLE IF NOT EXISTS inscription_participant_qr_tokens (
  id SERIAL PRIMARY KEY,
  participant_id INTEGER NOT NULL,
  evenement_id INTEGER NOT NULL,
  qr_token TEXT NOT NULL UNIQUE, -- Token unique généré à l'inscription
  ticket_url TEXT NOT NULL, -- URL complète du ticket avec token
  created_at TIMESTAMPTZ DEFAULT NOW(),
  expires_at TIMESTAMPTZ, -- Optionnel : date d'expiration du token
  ...
  -- Un seul token actif par participant par événement
...
-- Politique RLS (Row Level Security) si nécessaire
-- ALTER TABLE inscription_checkins ENABLE ROW LEVEL SECURITY;
-- ALTER TABLE inscription_participant_qr_tokens ENABLE ROW LEVEL SECURITY;
...
WHERE table_name IN ('inscription_checkins', 'inscription_participant_qr_tokens')
